...
function displayProducts(products) {
    ...
    products.forEach(product => {
        ...
        productDiv.innerHTML += `
            ...
            <img src="${product.image}" alt="${product.title}" style="max-width: 100px; height: auto" title="${product.title} class="hoverimage">
            <h3 class="rating">Stars ⭐: ${product.rating.rate}</h3>
            <h3 class="rating">Count: ${product.rating.count}</h3>
        `;
        const hoverDiv = document.createElement('div');
        hoverDiv.classList.add('hover-details');
        hoverDiv.innerHTML += `Description: <p>${product.description};`
        ...
        productDiv.addEventListener('mouseleave', function () {
            ...
            hoverDiv.style.top = '20px';
        });
        
        hoverDiv.addEventListener('transitionend', function () {
...
function proceedToCheckout() {
    if (cart.length > 0) {
        const total = cart.reduce((sum, product) => sum + product.price + 52.48, 0);
